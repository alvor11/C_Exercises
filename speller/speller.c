...
int main(int argc, char *argv[])
{
    ...
    char word[LENGTH+1];
    for (int c = fgetc(fp); c != EOF; c = fgetc(fp))// spell-check each word in text
    {
        if (isalpha(c) || (c == '\'' && index > 0)) // allow only alphabetical characters and apostrophes
        {
            ...
            {
                while ((c = fgetc(fp)) != EOF && isalpha(c));// consume remainder of alphabetical string
                index = 0;// prepare for new word
            }
        }
        else if (isdigit(c)) // ignore words with numbers (like MS Word can)
        {
            while ((c = fgetc(fp)) != EOF && isalnum(c));// consume remainder of alphanumeric string
            index = 0; // prepare for new word
        }
